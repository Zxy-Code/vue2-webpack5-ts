...
const WebpackUploadPlugin = require('../src/libs/js/webpack-upload-plugin.js');
...
if (process.env.SUPPORT_REMOTE) {
    ...
    const RD_NAME = process.env.SUPPORT_REMOTE_ADDRESS || 'zxy';
    if (remotes[RD_NAME] && remotes[RD_NAME].receiver) {
        plugins.push(
            new WebpackUploadPlugin({
                // 上传服务接口，插件会把文件逐个post到上传服务
                receiver: remotes[RD_NAME].receiver,
                to: remotes[RD_NAME].address,
    ...
    configureWebpack: config => {
        return {
            output: {
                ...
                // 产出文件名
                filename: path.join(assetsSubDirectory, 'js/[name].[fullhash].js'),
